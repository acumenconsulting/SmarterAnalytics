...
class SmarterAnalyticsAdmin {
    ...
    public function __construct() {
        ...
        add_action( 'admin_head', array( $this, 'stylesheet' ) );
    }
    ...
    public function create_admin_page() {
        ...
        <div class="wrap smarter-analytics-admin">
            ...
            <form method="post" action="options.php">
                ...
                <table class="wp-list-table widefat fixed pages smarter-analytics-admin-table" cellspacing="0" style="width:600px;">
                    ...
                    foreach ($codes as $code) {
                        if ($code != "") {
                            ...
                            echo '<tr id="smarter-analytics-code-' . $count . '" class="alternate">
                                    <td><label>' . $label . '</label></td>
                                    <td class="inline-edit-row">
                                        <span class="input-text-wrap">
                                            <input class="smarter-analytics-code" type="text" value="' . $code .'" />
                                        </span>
                                    </td>
                                    <td class="actions">
                                        <button class="button button-default remove-existing-code">Remove</button>
                                    </td>
                                </tr>';
                            $count++;
                        }
                ...
                <?php
                echo '<p class="submit"><input type="button" id="add-code-button" class="button button-default" value="Add New Google Analytics Code" /></p>';
                ?>
                ...
                <table class="wp-list-table widefat fixed pages smarter-analytics-user-exclusions-table" cellspacing="0" style="width:600px;">
                    ...
                    <tbody>
                        ...
                        <?php
                        $user_tracking_exclusions = $this->track_user_row("Administrator", $user_tracking_exclusions);
                        $user_tracking_exclusions = $this->track_user_row("Editor", $user_tracking_exclusions);
                        $user_tracking_exclusions = $this->track_user_row("Author", $user_tracking_exclusions);
                        $user_tracking_exclusions = $this->track_user_row("Contributor", $user_tracking_exclusions);
                        $user_tracking_exclusions = $this->track_user_row("Subscriber", $user_tracking_exclusions);
                        ?>
                        <tr>
    ...
    /**
     * Generate the select dropdown code for a given settings field ID 
     */
    ...
    /**
     * Render HTML for the tracked user row
     */
    private function track_user_row($user_type, $tracking_exclusions) {
        $tracking_exclusions_array = explode(",", $tracking_exclusions);
        $user_type_lower = strtolower($user_type);

        echo '<tr>';
        echo '<td class="track-user-label alternate">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;' . $user_type . '</td>';
        echo '<td>';
        $checked = in_array($user_type_lower, $tracking_exclusions_array);
        echo '<input type="checkbox" class="track-user" data-usertype="' . $user_type_lower .'"' . ($checked == true ? " checked" : " ") . ' />';
        echo '</td>';
        echo '</tr>';

        $tracking_exclusions = str_replace($user_type_lower . ",", "", $tracking_exclusions);
